...
const Post =({post,setCurrentId, page})=>{
    ...
    return(
        <Card className={classes.card} raised elevation={5}>
            ...
            <CardContent>
                <Typography variant="body2" color="textSecondary" component="p">{post.message.split(' ').splice(0, 20).join(' ')}... more</Typography>
            </CardContent>
            </ButtonBase>
            <CardActions className={classes.cardActions}>
                ...
                {(user?.result?.googleId === post?.creator || user?.result?._id === post?.creator) && (
                    <Button size="small" color="secondary" onClick={() => setOpenConfirmation(true)}>
            ...
            <Dialog open={openConfirmation} onClose={() => setOpenConfirmation(false)}>
                ...
                <DialogActions className={classes.box}>
                    ...
                    <Button variant="outlined" onClick={handleDeletePost} color="primary" autoFocus>
                        Yes
                    </Button>
                </DialogActions>
            </Dialog>
